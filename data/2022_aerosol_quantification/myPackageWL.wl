myExportFunctionList = {myAroundString, myBAUAColors, myCMYKColors, 
     myExportEquationList, myExportEquations, myExportF, myExportFigureList, 
     myExportFPNG, myExportFSVG, myFlatten, myFontSize, myFontStandard, 
     myFrameStyle, myFrameTickStyle, myLabelStyle, myLegendAppearance, 
     myPackageInputPrint, myPlotColors, myPlotSequence, myPlotStyle, 
     myPureColors, myReplaceList, myStringJoin, myStringSplit, 
     myStringSplit202507, myStyle, mySubJoin, myT, myTake, myTickStyle, 
     myTranspose}
...
myExportEquations[l_List] := Module[{id, eq}, 
     ((id = #1[[2]]; eq = #1[[1]]; Export[exPath~~"\\equations\\"~~id~~
            ".txt", ExportString[eq, {"MathML", "Expression"}]]; 
          If[MemberQ[myExportEquationList[[All,1]], id], 
           myExportEquationList = ReplacePart[myExportEquationList, 
              FirstPosition[myExportEquationList[[All,1]], id][[1]] -> {id, 
                eq}]; , AppendTo[myExportEquationList, {id, eq}]]; ) & ) /@ l*
       Export[exPath~~"\\equations\\##1 equation IDs and equations.pdf", 
        Column[(Row[Riffle[{#1[[1]], #1[[2]]}, "\t"]] & ) /@ 
          myExportEquationList]]; 
      Export[exPath~~"\\equations\\##2 equation IDs.rtf", 
       StringRiffle[Prepend[myExportEquationList[[All,1]], 
         exPath~~"\\equations\\"], "\n"]]; ]
 
myExportF[l_List] := Module[{r, g, nameID, name, icon}, 
     ((g = #1[[1]]; nameID = #1[[2]]; name = #1[[3]]; If[#1[[5]] == True, 
          r = 600; Table[Export[exPath~~"CMYK "~~ToString[r]~~" dpi\\"~~
              nameID~~" _ "~~ToString[r]~~" dpi CMYK"~~i, ColorConvert[
              Rasterize[g, ImageResolution -> r], "CMYK"]], 
            {i, {".eps", ".png"}}], Nothing]; r = 72; 
         icon = Rasterize[g, ImageResolution -> r]; 
         Table[Export[exPath~~StringReplace[ToString[i], "." -> ""]~~" "~~
            ToString[r]~~" dpi\\"~~nameID~~" _ "~~ToString[r]~~" dpi"~~i, 
           icon], {i, {".png"}}]; r = 150; 
         Table[Export[exPath~~StringReplace[ToString[i], "." -> ""]~~" "~~
            ToString[r]~~" dpi\\"~~nameID~~" _ "~~ToString[r]~~" dpi"~~i, 
           Rasterize[g, ImageResolution -> r]], {i, {".png"}}]; r = #1[[4]]; 
         Table[Export[exPath~~StringReplace[ToString[i], "." -> ""]~~" "~~
            ToString[r]~~" dpi\\"~~nameID~~" _ "~~ToString[r]~~" dpi"~~i, 
           Rasterize[g, ImageResolution -> r]], {i, {".png"}}]; 
         (Table[Export[exPath~~StringReplace[ToString[i], "." -> ""]~~"\\"~~
              nameID~~i, g], {i, {".svg", ".eps", ".pdf"}}] & ) /@ #1; 
         If[MemberQ[myExportFigureList[[All,1]], nameID], 
          myExportFigureList = ReplacePart[myExportFigureList, 
             FirstPosition[myExportFigureList[[All,1]], nameID][[1]] -> 
              {nameID, name, icon}]; , AppendTo[myExportFigureList, 
           {nameID, name, icon}]]; ) & ) /@ l; 
      Export[exPath~~"##1 full names of figure IDs.pdf", 
       Grid[Prepend[myExportFigureList, {"ID ", "figure name", "preview"}]]]; 
      Put[myExportFigureList, exPath~~"myExportFigureList.m"]; 
      Export[exPath~~"##2 figure IDs.rtf", StringRiffle[
        myExportFigureList[[All,1]], "\n"]]; ]
...
myPackageInputPrint[] := Module[{l}, 
     l = ToExpression /@ (Select[Names["Global`*"], StringContainsQ[
          ...
          "myExportFigureList" -> Nothing, "myExportEquationList" -> Nothing, 
          "myFontSize" -> Nothing, "myFontStandard" -> Nothing, 
          "myColors" -> Nothing, "myFigureIDs" -> Nothing, 
          "myColorForGradient" -> Nothing, "myPlotColors" -> Nothing}); 
      TableForm[Definition /@ l]]
 
myPlotSequence[] := Sequence[ImageSize -> Large, Frame -> True, 
     myFrameStyle[], myFrameTickStyle[]]
 
myPlotStyle[opacity_:1, colorList_List:myPlotColors] := 
    Module[{}, PlotStyle -> (Directive[Opacity[opacity], #1] & ) /@ colorList]
...
myStringSplit[s_String, deli_List] := Module[{i, out, j}, 
     i = 1; out = StringSplit[s, deli[[i]], All]; 
      For[j = 1, j < Length[deli], j++, 
       i++; out = Map[StringSplit[#1, deli[[i]], All] & , out, {i - 1}]]; out]
 
myStringSplit202507[s_String, deli_List] := Module[{i, out, j}, 
...
myStyle[s_, fontsize_Integer:myFontSize, fontfamily_String:myFontStandard, 
     color_:GrayLevel[0]] := Module[{}, Style[If[StringQ[s], s, ToString[s]], 
